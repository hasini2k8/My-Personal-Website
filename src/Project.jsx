...
const BOOKS = [
  {
    ...
    // This book's own project-images panel — position (top/left, %
    // of viewport, CENTER of the card), size (width/height, px), and
    // tilt (tiltDeg, degrees — negative = counter-clockwise). Every
    // book has its own copy of these now, so each panel can sit
    // somewhere different, be a different size, and tilt a different
    // way. Any field you omit falls back to the shared PROJECT_IMAGES_
    // PANEL_* defaults further down.
...
/* ====== MAGNIFY ICON (sits on the current project-images thumbnail) ======
   ...
   Clicking it pops THAT SAME image — whichever one you're on — into a
   bigger lightbox. top/left are a % of the panel's own image frame, so
   the icon stays put on the art regardless of image size. Move/resize
   freely. */
...
/* ====== LIGHTBOX (expanded project-images thumbnail) ======
   Full-screen viewer opened by the magnify icon on the project-images
   panel. Shows whichever thumbnail (1, 2, 3...) was active when
   clicked, larger and centered, with its own ✕. */
...
export default function Project() {
  ...
  // Desktop project-images scroll panel — which image (0-indexed) is
  // currently showing. Resets whenever the active book changes so a new
  // book always starts back at its first image.
  const [projectImageIndex, setProjectImageIndex] = useState(0);

  // Lightbox — expanded view of the ink-revealed (22.png) image,
  // opened by tapping the magnify icon that sits on top of it.
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ===== DESKTOP: project-images scroll panel (shown once the
           22.png ink reveal is up) ===== */
        ...
        /* ===== MAGNIFY ICON — sits on top of the ink-revealed (22.png)
        ...
        /* ===== LIGHTBOX — expanded ink-reveal image ===== */
        .lightbox-backdrop {
          position: fixed;
          inset: 0;
          z-index: 60;
              ...
              <div className="project-images-frame">
                {activeBook.projectImagesCount > 1 && (
                  <button
                    ...
                    onClick={() => handlePrevProjectImage(activeBook.projectImagesCount)}
                ...
                {activeBook.projectImagesCount > 1 && (
                  <button
                    ...
                    onClick={() => handleNextProjectImage(activeBook.projectImagesCount)}
